...
export class CheckoutActions extends CheckoutPage {
  async chooseGuestAndContinueIfShown(): Promise<void> {
    ...
    if (await guestRadio.count()) {
      ...
      await Promise.all([this.page.waitForLoadState('domcontentloaded'), continueBtn.click()]);
    }
  }
  ...
  async confirmOrder(): Promise<void> {
    ...
    await Promise.all([this.page.waitForURL(/rt=checkout\/success/i), confirmBtn.click()]);
  }
}
